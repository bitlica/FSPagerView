...
extension FSPagerView {
    public var centeredCollectionViewPoint: CGPoint {
        return CGPoint(
    ...
    public var centeredCollectionViewIndexPath: IndexPath? {
        return collectionView
            .indexPathForItem(at: centeredCollectionViewPoint)
    }
    
    public func cellFrameForItem(at indexPath: IndexPath) -> CGRect? {
        let attributes = collectionView.layoutAttributesForItem(at: indexPath)
        
        return attributes?.frame
    }
    
    public var highlightedItemArea: CGRect {
        let origin = CGPoint(
            x: centeredCollectionViewPoint.x - itemSize.width / 2.0,
            y: 0
        )
